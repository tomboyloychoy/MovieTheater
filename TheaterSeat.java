    ...
    private String chooseBestSeat(String request, int numSeat){
        ...
        for (char i : bestRow){
            if (theaterMap.get(i).size() >= numSeat && seatFound == false){
                seatFound = true;
                row = i;
                // Find suitable seat for request
                ArrayList<String> seat = new ArrayList<>();
                for (int k = 0; k < numSeat; k++){
                    seat.add(row.toString() + theaterMap.get(i).get(k));
                }

                removeSeat(numSeat, row);
                // buffer seat
                int buffer = 3;
                if (theaterMap.get(i).size() > 3){
                    while (buffer > 0){
                        buffer -= 1;
                        removeSeat(1, row);
                    }
        ...
        if (seatFound == false){
            for (char i : theaterMap.keySet()) {
                if (theaterMap.get(i).size() >= numSeat){
                    ...
                    // buffer seat
                    int buffer = 3;
                    while (buffer > 0 || theaterMap.get(i).size() > 0){
                            buffer -= 1;
                            removeSeat(1, row);
                    }
                    bookedSeat.put(request, seat);
